...
var getPlaylist = function(id) {
	...
	return exists.then(function(playlistString) {
		...
		responses = _(urlParams).map(function(param) {
		...
		return Q.allResolved(responses).then(function(promises) {
			list = _.chain(promises).map(function(promise, i) {
...
var savePlaylist = function(playlist) {
	...
	var result = Q.defer(), verified = verifyPlaylist(playlist);
	verified.fail(function() {
