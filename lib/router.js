    ...
    testUrl: function(input) {
        ...
        if (httpMatch) {
            var prefix = httpMatch[2] || '',
                downloadMatch = input.match(downloadFormat);
            if (downloadMatch) {
...
Router.prototype = {
    get: function(url, mediaHandler, excludedSites) {
        ...
        if (url) {
            if (isURL || url instanceof KeyValuePair) {
                ...
                if (func) {
                    deferred = func(url).then(mediaHandler);
                    success = true;
                }
            }
        }
        if (!success) {
            if (url && !isURL && this.verifyUrl('http://' + url)) {
    ...
    testResource: function(input, exclusions) {
        ...
        var route, result = null, possibleRoutes = [], i, j;
        possibleRoutes = _.reject(this.routingTable, function(route) {
            return _.contains(exclusions, route.siteName);
        });
        route = _.find(possibleRoutes, function(route) {
            var accepted = route.test(input);
            return accepted;
        });
