...
var Router = function(soundcloudConsumerKey, youtubeKey) {
    ...
    this.buildRoutingTable = function() {
        var router = this;
        var table = [];
        table.push(getMediaRoute(soundcloudTrackOptions));
        table.push(getMediaRoute(youtubeVideoOptions));
        /*table.push({
            site: 'Reddit',
            test: function(input) {
...
Router.prototype = {
    ...
    processInternetLink: function(url, deferred, params) {
        var router = this;
    ...
    processRedditLink: function(url, deferred, params) {
        var router = this;
        if (url instanceof KeyValuePair) {
    ...
    processSoundCloudPlaylist: function(playlistID, deferred, params) {
            ...
            if (data.streamable === true) {
                ...
                if (data.tracks && data.tracks.length > 0) {
                    for (var index in data.tracks) {
                        ...
                        var deferredAction = $.Deferred();
                        router.processSoundCloudTrack(track, deferredAction, params);
    ...
    processSoundCloudTrack: function(trackID, deferred, params) {
        var consumerKey = this.soundcloudConsumerKey;
        var router = this;
    ...
    processYouTubeVideoID: function(youtubeID, deferred, params) {
        var router = this;
    ...
    resolveInternetLink: function(url, deferred, params) {
        deferred || (deferred = $.Deferred());
        if (!params)
            params = {};
        this.processInternetLink(url, deferred, params);
        return deferred.promise();
    },
    resolveReddit: function(url, deferred, params) {
        deferred || (deferred = $.Deferred());
        if (!params)
            params = {};
        this.processRedditLink(url, deferred, params);
        return deferred.promise();
    },
    resolveSoundCloud: function(url, deferred, params) {
        var router = this, 
            resolveURL, 
        ...
        request(requestOptions, function(error, response, body) {
            ...
            //Tracks have stream URL
            if (body.stream_url) {
                router.processSoundCloudTrack(body, deferred, params);
            }
            else {
                router.processSoundCloudPlaylist(body, deferred, params);
            }
        });
        return deferred.promise();
    },
    resolveYouTube: function(url, deferred, params) {
        ...
        params || (params = {});
        this.processYouTubeVideoID(youtubeID, deferred, params);
        return deferred.promise();
    },
