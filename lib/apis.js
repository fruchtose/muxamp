...
SearchApi.prototype = {
	...
	sort: function(tracks, options) {
		...
		_(tracks).each(function(result) {
			...
			if (undefined === result.stats.plays) {
		...
		return _.chain(tracks).map(function(result) {
			...
			result.stats.playRelevance = Math.log(result.stats.plays + 1) / Math.log(maxPlays + 1);
			result.stats.favoriteRelevance = Math.log(result.stats.favorites + 1) / Math.log(maxFavorites + 1);
			result.calculateRelevance();
			return result;
		}).sortBy(function(a) {
			// Sorts tracks by relevance
			return a.stats.relevance;
...
var YouTube = new function() {
	...
	var getOne = function(entry) {
		...
		options = {
			...
			favorites: entry['yt$statistics']['favoriteCount'],
			icon: 'img/youtube.png',
			mediaName: entry.title.$t,
			permalink: permalink,
			plays: entry['yt$statistics']['viewCount'],
			siteCode: 'ytv',
			siteMediaID: id,
			type: 'video',
			url: permalink
