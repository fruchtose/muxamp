...
var YouTube = new function() {
    ...
    var parseDuration = function(durationString) {
        var minutes, seconds, pieces = durationString.match(/PT(\d+)M(\d+)S/);
    ...
    var trackStreamApi = new StreamApi({
        getOne: function(body) {
            if (!body.items && /too_many_recent_calls/.test(body)) {
                throw new QuotaError('YouTube');
            }
            var entry = body.items && body.items[0];
            if (!entry) {
                throw new Error('YouTube API response ' + JSON.stringify(body) + ' is invalid.');
            }
            if(!(entry.status && entry.status.embeddable)) {
                throw new Error('YouTube track not streamable.');
            }
            var url = 'http://www.youtube.com/watch?v=' + entry.id;
            var params = {
                author: entry.snippet.channelTitle,
                duration: parseDuration(entry.contentDetails.duration),
                icon: 'img/youtube.png',
                mediaName: entry.snippet.title,
                permalink: url,
                siteCode: 'ytv',
                siteMediaID: entry.id,
                type: 'video',
        ...
        url: function(options) {
            ...
            return 'https://www.googleapis.com/youtube/v3/videos?' + [
                'key=' + config.get('muxamp:apis:youtube:serverKey'),
                'id=' + id,
                'part=' + encodeURIComponent('snippet,contentDetails,status'),
                'fields=items'
            ].join('&');
        }
    });
